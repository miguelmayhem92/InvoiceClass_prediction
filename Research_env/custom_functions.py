...
def my_train_test_split_with_balance(data, test_size, n, threshold, seed):
    data['weigth_vector'] = data.groupby('Map_Product_Category').Inv_Id.transform('count')
    
    samples = list()
    for ind_target in data[data.weigth_vector >= threshold].Map_Product_Category.unique():
        df = data[data.Map_Product_Category == ind_target].sample(n = n, random_state = seed)
        samples.append(df)
...
class get_items_in_description(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X = X.copy()
        for feature, feature_name in zip(self.variables, self.new_variable_names):
            X[feature_name] = X.apply(lambda x: count_character(x[feature], '/'),axis = 1) + 1
        return X
...
class get_keywords_from_description(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X = X.copy()
        for feature, feature_name, regex in zip(self.variables, self.new_variable_names, self.keywords):
...
class get_date_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X = X.copy()
        for feature in self.variables:
            X['DateFound'] = X.apply(lambda x: find_pattern(x.Item_Description, self.date_regex),axis = 1)
            X['DateFound'] = X.apply(lambda x: get_date(x.DateFound, self.year_regex, self.moth_regex),axis = 1)
            X['DateFound_format'] = pd.to_datetime(X['DateFound'], format="%Y-%b") ## drop for a while
